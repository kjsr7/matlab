...
number_of_dimensions = 2 ;             %Number of dimensions the function has
c1 = 2.25 ;                             %Personal acceleration coefficient
c2 = 2.25 ;                             %Social acceleration coefficient
number_of_particles = 20 ;              %Number of particles in swarm
number_of_iterations = 2000 ;            %Number of iterations of PSO loop
min = -2 ;                           %Minimum vlaue of any dimension
max =  2 ;                           %Maximum value of any dimension
fitnessFunction = @(x,y) camel_back (x,y) ;  %Fitness function to be optimized
...
for i = 1 : number_of_particles
    ...
    swarm(i).fitness = fitnessFunction (swarm(i).position(1,1),swarm(i).position(1,2));            %Initialising the fitness of the swarm
...
for i = 1 : number_of_iterations
    for j = 1:number_of_particles
        ...
        swarm(j).fitness = fitnessFunction(swarm(j).position(1,1),swarm(j).position(1,2)) ;     %Fitness Update
